...
export default function PendingMatchApprovalsClient() {
  ...
  const handleApproveMatch = (matchId: string) => {
    ...
    const winnerIndex = teams.findIndex(
      (t: { name: string; wins: number; losses: number }) => t.name === winner
    );
    if (winnerIndex >= 0) {
    ...
    // Update or add loser
    const loserIndex = teams.findIndex(
      (t: { name: string; wins: number; losses: number }) => t.name === loser
    );
    if (loserIndex >= 0) {
  ...
  return (
    <>
      <h2>
        Pending Match Approvals <Badge bg="warning" className="ms-2">
            ...
            <Row className="align-items-center">
              <Col md={6}>
                ...
                <div className="text-muted small">
                  <div>
                    <strong>Game:</strong> {match.gameName}
                  </div>
                  <div>
                    <strong>Date:</strong>{' '}
                    {new Date(match.matchDate + 'T' + match.matchTime).toLocaleString()}
                  </div>
                  <div>
                    <strong>Submitted by:</strong> {match.submittedBy}
                  </div>
                </div>
              </Col>
              <Col md={3}>
                <Badge
                  bg={match.team1Score > match.team2Score ? 'success' : 'info'}
                  className="p-2"
                >
